...
class InstaloaderContext:
    ...
    def login(self, user, passwd):
        ...
        if resp_json.get('checkpoint_url'):
            # Instagram checkpoint/challenge requires JavaScript rendering.
            # The most reliable method is opening the URL in the user's real browser.
            import webbrowser
            checkpoint_url = resp_json.get('checkpoint_url')
            full_checkpoint_url = "https://www.instagram.com" + checkpoint_url if checkpoint_url.startswith('/') else checkpoint_url

            # Primary method: open in user's real browser (JS works → notification sent)
            try:
                webbrowser.open(full_checkpoint_url)
            except Exception:
                pass

            # Secondary: also try programmatic POST as a bonus attempt
            try:
                challenge_resp = session.get(full_checkpoint_url)
                updated_csrf = session.cookies.get_dict().get('csrftoken', csrf_token)
                session.headers.update({'X-CSRFToken': updated_csrf})
                session.post(full_checkpoint_url, data={'choice': 0}, allow_redirects=True)
            except Exception:
                pass

            raise LoginException(
                f"Login: Checkpoint required. Browser opened to {checkpoint_url} - "
